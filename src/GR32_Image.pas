...
(*
  ...
  -Add mode scaled by width and height
...
type
  ...
  TScaleMode = (smNormal, smStretch, smScale, smResize, smOptimal, smOptimalScaled, smFitWidth, smFitHeight);
...
{$ENDIF}
  ...
  public
    ...
    procedure ScrollToMouseMove(X, Y: Single);
    ...
    property HorScroll: TCustomRangeBar read HScroll;
    property VerScroll: TCustomRangeBar read VScroll;
  end;
  ...
  begin
    ...
    case Mode of
      ...
      smFitWidth:
        begin
          Cy := Bitmap.Height;
          RScaleX := ViewportWidth / Bitmap.Width;
          Cx := ViewportWidth;
          Cy:=Round(Cy * RScaleX);
        end;
      smFitHeight:
        begin
          Cx := Bitmap.Width;
          RScaleY := ViewportHeight / Bitmap.Height;
          Cx := Round(Cx * RScaleY);
          Cy := ViewportHeight;
        end;
      smResize:
...
procedure TCustomImgView32.ScrollToMouseMove(X, Y: Single);
begin
  DisableScrollUpdate := True;
  HScroll.Position := HScroll.Position + X;
  VScroll.Position := VScroll.Position + Y;
  DisableScrollUpdate := False;
  UpdateImage;
end;
