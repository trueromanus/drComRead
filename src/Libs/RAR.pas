...
begin
  ...
  try
    ...
    while (ReadFileHeaderResult=RAR_SUCCESS) and Result do begin
      ...
      if extractFile(StrPas(hdrData.FileNameW),Files) then begin    //todo: UniCode FileName

        if RestoreFolder then
        begin
          ExistentFile.FileName:=Path+StrPas(hdrData.FileNameW);
          ExFileName:=Path+hdrData.FileNameW
        end else
        begin
          ExistentFile.FileName:=Path+ExtractFileName(hdrData.FileNameW);
          ExFileName:=Path+ExtractFileName(hdrData.FileNameW);
        end;
        ...
        case ReplaceResult of
          ...
          rrOverwrite: if RestoreFolder then
                        ReadFileHeaderResult:=RARProcessFileW(ArchiveHandle, RAR_EXTRACT, nil,PWideChar(ExFileName))
                      else
